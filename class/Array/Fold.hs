...
instance Fold (S# Latin1) Char ((a ∷ K A) → a) where {\
    fold (S# s) f r0 = go (P# @Char8# s) r0 \
      where go s r = let !(Char8# ch) = s ! 0# in if cast (ch ≡ '\0'#) then r else cast ch `f` go (s +. 1#) r}
...
INST_FOLD_LATIN(F64)
INST_FOLD_LATIN((##))
INST_FOLD_LATIN(Bytes)

unpackFoldrCStringUtf9# ∷ S# UTF8 → (Char → t → t) → t → t
unpackFoldrCStringUtf9# (S# p0) f r0 = go (P# @Char8# p0) r0
  where
    go p r = let !(Char8# ch) = p ! 0#
                in if cast (ch ≡ '\0'#) then r else
          let !n = byteCount ch
          in C# (unpackUtf8Char# n ch p) `f` go (p +. n) r
...
instance P_Unbox Char8# +. ByteCount where
  (+.) p = \case One   -> p +. 1#
                 Two   -> p +. 2#
                 Three -> p +. 3#
                 Four  -> p +. 4#
...
unpackUtf8Char# :: ByteCount -> Char# -> P_Unbox Char8# -> Char#
unpackUtf8Char# bytes ch'@(cast @I → ch) p =
  case bytes of
    One   -> ch'
    Two   -> cast (((ch - 0xC0#) <<#  6##) + (cast ((p +. 1#) ! 0#) - 0x80#))
    Three -> cast ( ((           ch - 0xE0#) <<# 12##)
                  + ((cast (p ! 1#) - 0x80#) <<#  6##)
                  + ( cast (p ! 2#) - 0x80#          ))
    Four  -> cast ( ((           ch - 0xF0#) <<# 18##)
                  + ((cast (p ! 1#) - 0x80#) <<# 12##)
                  + ((cast (p ! 2#) - 0x80#) <<#  6##)
                  + ( cast (p ! 3#) - 0x80#          ))
