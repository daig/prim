...
#define INST_REF(TY,GET,READ,WRITE) \
instance (x ≑ TY) ⇒ Ref (x) P_Unbox where { \
  get = coerce (`GET#` 0#) ;\
  read = coerce (`READ#` 0#) ;\
  (.=) = coerce (`WRITE#` 0#) }

#define INST_REF_SPEC(TY,GET,READ,WRITE) \
instance {-# OVERLAPPING #-} Ref (TY) P_Unbox where { \
  get = coerce (`GET#` 0#) ;\
  read = coerce (`READ#` 0#) ;\
  (.=) = coerce (`WRITE#` 0#) }

INST_REF(I,indexIntOffAddr,readIntOffAddr,writeIntOffAddr)
INST_REF(I8,indexInt8OffAddr,readInt8OffAddr,writeInt8OffAddr)
INST_REF(I16,indexInt16OffAddr,readInt16OffAddr,writeInt16OffAddr)
INST_REF(I32,indexInt32OffAddr,readInt32OffAddr,writeInt32OffAddr)
INST_REF(I64,indexInt64OffAddr,readInt64OffAddr,writeInt64OffAddr)
INST_REF(U,indexWordOffAddr,readWordOffAddr,writeWordOffAddr)
INST_REF(U8,indexWord8OffAddr,readWord8OffAddr,writeWord8OffAddr)
INST_REF(U16,indexWord16OffAddr,readWord16OffAddr,writeWord16OffAddr)
INST_REF(U32,indexWord32OffAddr,readWord32OffAddr,writeWord32OffAddr)
INST_REF(U64,indexWord64OffAddr,readWord64OffAddr,writeWord64OffAddr)
INST_REF(Addr#,indexAddrOffAddr,readAddrOffAddr,writeAddrOffAddr)
INST_REF_SPEC(Char8#,indexCharOffAddr,readCharOffAddr,writeCharOffAddr)
INST_REF_SPEC(Char#,indexWideCharOffAddr,readWideCharOffAddr,writeWideCharOffAddr)
