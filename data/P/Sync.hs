...
get' ∷ ∀ a. Sync# a → IO (# (##) | a #) {- ^ The value if the @Sync.P@ was full -}
...
peek' ∷ ∀ a. Sync# a → IO (# (##) | a #) {- ^ The value if the @Sync.P@ was full/unlocked -}
...
put' ∷ Sync# a → a → IO B# {- ^ whether the write succeeded -}
