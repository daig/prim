...
instance Copy Bytes (MutableAddr# s)  s where
 copy (coerce → src) i (coerce → dst) j n = copyByteArrayToAddr# src i (plusAddr# dst j) n
instance Copy (Bytes_M s) (MutableAddr# s) s where
...
instance Copy (MutableAddr# s) (Bytes_M s) s where
...
instance Copy (A_Unbox_Pinned_M x s) (P_Unbox x) s where
 copy src i (Addr# dst) j n = coerce copyMutableByteArrayToAddr# src i (plusAddr# dst j) n
